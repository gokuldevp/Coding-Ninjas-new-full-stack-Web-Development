...
server.post('/', (req, res) => {
  res.status(201).send('The POST method submits an entity to the specified resource, often causing a change in state or side effects on the server.');
});

// Handle PUT request at the root path
server.put('/', (req, res) => {
  res.set('Content-Type', 'text/plain');
  res.status(200).send('The PUT method replaces all current representations of the target resource with the request content.');
});

// Handle DELETE request at the root path
server.delete('/', (req, res) => {
  res.set('Content-Type', 'text/plain');
  res.status(202).send('The DELETE method deletes the specified resource.');
});
